...
boxcox(wpm ~ window+id, data = data_LI_wpm)
...
## Means by window (by subject and by item)

```{r}
data_LI_wpm |> 
  group_by(id, window_reordered) |> 
  reframe(wpm=mean(wpm, na.rm=TRUE)) |> 
  group_by(window_reordered) |> 
  reframe(N=n(), wpm_m=mean(wpm), wp_sd=sd(wpm))

data_LI_wpm |> 
  group_by(sentence_id, window_reordered) |> 
  reframe(wpm=mean(wpm, na.rm=TRUE)) |> 
  group_by(window_reordered) |> 
  reframe(N=n(), wpm_m=mean(wpm), wp_sd=sd(wpm))
```
...
```{r}
data_LI_wpm$window <- data_LI_wpm$window_reordered
contrasts(data_LI_wpm$window) <- contr.helmert(4)

mm <- model.matrix(wpm ~ 1+window, data_LI_wpm)
data_LI_wpm$wc0 <- mm[, 4]
data_LI_wpm$wc3 <- mm[, 3]
data_LI_wpm$wc2 <- mm[, 2]
...
isSingular(wpm_model_LI_1)      # FALSE
summary(rePCA(wpm_model_LI_1))  # Not ok for sentence_id
VarCorr(wpm_model_LI_1)
```

IN: The model has a singularity issue.
RK: Probably a false positive. The model converges but is overparameterized.

### Fitting a model with uncorrelated random slope and intercept (sentence_id)

RK: You cannot use "||" with factors! I replaced the factor with indicator variables.

```{r}
wpm_model_LI_1.1 <- lmer(
  sqrt(wpm) ~ 1 + window + (1 + window | id) + (1 + wc0 + wc3 + wc2 || sentence_id),
  data = data_LI_wpm,
  REML = FALSE,
  control=lmerControl(calc.derivs=FALSE)
)

isSingular(wpm_model_LI_1.1)      # TRUE
summary(rePCA(wpm_model_LI_1.1))  # Not ok Sentence ID
VarCorr(wpm_model_LI_1.1)

anova(wpm_model_LI_1.1, wpm_model_LI_1)
```

IN: The model failed to converge. Removing slope for sentence_id
RK: Here we have a singularity issue. But there was no loss of information


```{r}
wpm_model_LI_2 <- lmer(
  sqrt(wpm) ~ 1 + window + (1 + window | id) + (1 | sentence_id),
  data = data_LI_wpm,
  REML = FALSE,
  control=lmerControl(calc.derivs=FALSE)
)

VarCorr(wpm_model_LI_2)
isSingular(wpm_model_LI_2)      # TRUE
summary(rePCA(wpm_model_LI_2))  # Not ok Sentence ID
```

IN: The model has a singularity issue. 
RK: CP is at boundary -1.00
...
wpm_model_LI_2.1 <- lmer(
  sqrt(wpm) ~ 1 + wc0 + wc3 + wc2 + (1 + wc0 + wc3 + wc2 || id) + (1 | sentence_id),
  data = data_LI_wpm,
  REML = FALSE,
  control=lmerControl(calc.derivs=FALSE)
)

VarCorr(wpm_model_LI_2.1)
isSingular(wpm_model_LI_2.1)    # FALSE
summary(rePCA(wpm_model_LI_2.1))  # Not ok Sentence ID
print(summary(wpm_model_LI_2.1), cor=FALSE)
```

IN: The model failed to converge. Removing both slopes
RK: This model might be ok, but one of the PCs is very small. 


```{r}
wpm_model_LI_3 <- lmer(
  sqrt(wpm) ~ 1 + wc0 + wc3 + wc2 + (1 | id) + (1 | sentence_id),
  data = data_LI_wpm,
  REML = FALSE,
  control=lmerControl(calc.derivs=FALSE)
)

VarCorr(wpm_model_LI_3)
isSingular(wpm_model_LI_3)    # FALSE
summary(rePCA(wpm_model_LI_3))  # Not ok Sentence ID

anova(wpm_model_LI_3, wpm_model_LI_2.1, wpm_model_LI_2, wpm_model_LI_1.1, wpm_model_LI_1)
```

RK: Significant loss in goodness of fit according to LRT, AIC, and BIC. The preferred LMM in this sequence is wpm_model_LI_2.1. However, there may be parameterizations that take care of overparameterization of 


## Assumptions
```{r}
check_model(wpm_model_LI_2.1,
            panel = TRUE, 
...
data_LI_reg_c |> 
  group_by(id, window_reordered) |> 
  reframe(reg_count=mean(reg_count, na.rm=TRUE)) |> 
  group_by(window_reordered) |> 
  reframe(N=n(), reg_count_m=mean(reg_count), wp_sd=sd(reg_count))
...
```{r}
data_LI_reg_c$window <- data_LI_reg_c$window_reordered
contrasts(data_LI_reg_c$window) <- contr.helmert(4)
```
...
```{r}
reg_c_glmm_LI_1.1 <- glmer(reg_count ~ window + (1 + window | id) + (1 + wc0 + wc3 + wc2 || sentence_id), 
                      data = data_LI_reg_c,
                      family = 'poisson')
```
```{r}
...
```
```{r}
reg_c_glmm_LI_2.1 <- glmer(reg_count ~ window + (1 + wc0 + wc3 + wc2 || id) + (1 | sentence_id), 
                      data = data_LI_reg_c,
                      family = 'poisson')
```
```{r}
...
```{r}
library(arrow)
write_feather(data.frame(data_LI_wpm), "data_LI_wpm.arrow")
write_feather(data.frame(data_LI_reg_c), "data_LI_reg_c.arrow")
```
